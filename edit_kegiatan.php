...
$kegiatan = query("SELECT * FROM kegiatan WHERE id = $id");
...
if (isset($_POST["submit"])) {
    ...
    // query sql
    $query = "UPDATE kegiatan SET judul='$judul', isi='$isi', tempat='$tempat', tanggal='$hari_ini' where id='$id'";
    $hasil=mysqli_query($conn, $query);
    
    header("location: admin.php?halaman=kegiatan1");

    //cek apakah data berhasil ditambahkan atau tidak
    if (editKegiatan($_POST) > 0) {
        echo "
            <script>
                alert('Berhasil Mengedit Kegiatan');
            </script>
        ";
    } else {
        echo "
            <script>
                alert('Gagal Mengedit Kegiatan');
            </script>
        ";
    }
}
        ...
        <div class="container">
            <table class="table table-bordered text-center">
                ...
                <div class="form-group row font-weight-bold text-uppercase">
                    ...
                    <div class="col-sm-10">
                        <input value="<?php echo $row['judul'];?>" type="text" name="judul">
                    </div>
                </div>
                <div class="form-group row font-weight-bold text-uppercase">
                    <label for="tanggal" class="col-sm-2 col-form-label text-black">tanggal</label>
                    <div class="col-sm-10">
                        <input value="<?php echo $row['tanggal'];?>" type="text" name="tanggal">
                    </div>
                </div>
            <div class="form-group row font-weight-bold text-uppercase">
                    <label for="tempat" class="col-sm-2 col-form-label text-black">tempat</label>
                    <div class="col-sm-10">
                        <input value="<?php echo $row['tempat'];?>" type="text" name="tempat">
                    </div>
                </div>
                 <div class="form-group row">
                     <label for="isi" class="col-sm-2 col-form-label text-black">Isi</label>
                    <div class="col-sm-10">
                    <textarea class="ckeditor" id="summernote" name="isi" ><?php echo $row['isi'] ?></textarea>
                    </div>
                </div>
                <div class="form-group row font-weight-bold text-uppercase">
